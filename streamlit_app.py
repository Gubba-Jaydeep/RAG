...
def load_data(csv_path):
    ...
    df = df[df['Year'] >= 2022]  # Filter last two years
...
# Query function with Re-Ranking and Confidence Score
def query_rag(query, bm25, index, snippets, embeddings, top_k=5):
    ...
    return f"Financial insight: {best_doc[1]}\nConfidence Score: {best_doc[0]:.2f}"


# Streamlit App
def main():
    st.title("RAG Web App: Re-Ranking with Cross-Encoders")
    st.write("Ask financial-related questions and get insights from financial statements.")
    ...
    if st.button("Search"):
        ...
        if not is_valid:
            ...
            df, snippets = load_data('financial_statements.csv')
